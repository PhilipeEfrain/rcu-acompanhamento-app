...
export const symptomRepository = {
  async save(entry: DailySymptomEntry): Promise<DailySymptomEntry> {
    ...
    const row: InsertDailySymptomLog = {
      ...
      bloodAspect: entry.bloodAspect || null,
  ...
  async getLogsForDate(date: string): Promise<DailySymptomEntry[]> {
    ...
    return rows.map((r) => ({
      ...
      outputType: (r.outputType as OutputType) || undefined,
      period: (r.period as TimePeriod) || undefined,
      bloodAspect: (r.bloodAspect as BloodAspect) || undefined,
