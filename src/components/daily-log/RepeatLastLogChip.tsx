...
export const RepeatLastLogChip: React.FC<RepeatLastLogChipProps> = ({
  ...
  const getSummary = () => {
    if (lastLog.outputType === 'blood_mucus_only') {
      return t('dailyLog:repeatModal.chipSummaryBloodOnly', { pain: lastLog.painLevel });
    }
    const bristolLabel = t(`dailyLog:bristol.${lastLog.bristolType}.label`).split(':')[0];
      ...
      >
        ...
        <View style={styles.textContainer}>
          <Text style={styles.titleText}>
            {t('dailyLog:repeatModal.chipTitle', { time: lastLog.time || '--:--' })}
          </Text>
